...
### 关键概念区分
1. **定时器触发频率** (Timer Trigger Frequency): TIM2产生触发信号的频率
2. **ADC总采样率** (ADC Total Sample Rate): 所有通道的采样点总和速率
3. **单通道采样率** (Single Channel Sample Rate): 每个通道的实际采样率
...
因此：
- **最大定时器触发频率**: 1/3.86μs ≈ 259kHz
- **最大单通道采样率**: 259kHz ÷ 3 ≈ 86.3kHz
...
### Tim2_Config_Channel_Fs(float target_channel_fs)
- **功能**: 直接设置目标单通道采样率
- **约束**: 不超过硬件最大采样率
- **返回**: 实际单通道采样率
