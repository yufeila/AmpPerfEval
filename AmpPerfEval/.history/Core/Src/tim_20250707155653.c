...
/* ----------------------------------------------------------
 * 智能采样率选择函数，根据信号频率自动选择最优采样率
 * 输入：signal_freq - 信号频率 (Hz)
 * 返回：实际采样率 (Hz)
 * 
 * 采样率策略：
 * - ≤50kHz: 200kHz采样（保守配置，高稳定性）
 * - 50-100kHz: 400kHz采样（标准配置，AI推荐）
 * - >100kHz: 451kHz采样（极限配置，最大带宽）
 * ----------------------------------------------------------*/
float Tim2_Config_Smart_Fs(float signal_freq)
{
    float target_fs;
    
    if (signal_freq <= 50000.0f) {
        target_fs = SAMPLING_RATE_CONSERVATIVE;  // 200kHz
    } else if (signal_freq <= 100000.0f) {
        target_fs = SAMPLING_RATE_STANDARD;      // 400kHz
    } else {
        target_fs = SAMPLING_RATE_MAXIMUM;       // 451kHz
    }
    
    return Tim2_Config_Channel_Fs(target_fs);
}
