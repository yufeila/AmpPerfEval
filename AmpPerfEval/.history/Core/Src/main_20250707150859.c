...
/* USER CODE BEGIN PV */
// 模式控制变量
uint8_t basic_measurement = 1;      // 默认启动基本测量模式
uint8_t sweep_freq_response = 0;    // 扫频响应测量模式
uint8_t measure_R_out = 0;          // 输出阻抗测量标志
/* USER CODE END PV */
...
int main(void)
{
  ...
  while (1)
  {
    ...
    else if(current_system_state == MEASURE_R_OUT_STATE)
    {
        // 如果需要，可以在这里添加输出阻抗测量函数
        // Measure_R_Out();
    }
