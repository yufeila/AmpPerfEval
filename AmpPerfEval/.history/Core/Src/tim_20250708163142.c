...
float Tim2_Config_AutoFs(float f0_hz)
{
    ...
    if (f0_hz <= 20000.0f) {
        ...
        // 高频信号（>20kHz）：优先保证防混叠，但要受硬件约束
        // 使用更高的采样率倍数以提高测量精度，但不超过硬件极限
        float Fs_high = 5.0f * f0_hz;          // 高频时使用5倍采样率
        if (Fs_high > Fs_hw_max) {
            Fs_high = Fs_hw_max;               // 受硬件限制，最大254kHz
        }
    ...
    if (Fs_target > Fs_hw_max) {
        Fs_target = Fs_hw_max;                // 硬件约束优先，最大约259kHz
    }
    
    /* 额外安全检查：确保不超过ADC物理极限 */
    if (Fs_target > 250000.0f) {             // 强制限制在250kHz以下
        Fs_target = 250000.0f;
    }
