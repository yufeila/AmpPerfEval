...
/* ----------------------------------------------------------
 * 直接配置单通道采样率，返回实际达到的单通道采样率
 * 输入：target_channel_fs - 目标单通道采样率 (Hz)
 * 返回：实际单通道采样率 (Hz)
 * 注意：定时器触发频率 = 单通道采样率 × 通道数
 * ----------------------------------------------------------*/
float Tim2_Config_Channel_Fs(float target_channel_fs)
{
    /* 1. 检查硬件约束 */
    float max_channel_fs = TIM_MAX_TRIGGER_FREQ / ADC_CHANNELS;
    if (target_channel_fs > max_channel_fs) {
        target_channel_fs = max_channel_fs;
    }

    /* 2. 计算定时器触发频率 */
    float trigger_freq = target_channel_fs * ADC_CHANNELS;
