...
uint8_t fault_detection_flag = 0;        // 故障检测标志位
...
static uint8_t key_up_last_state = 1; // 默认按键未按下
...
void KEY_Scan(void)
{
    ...
    if (key_up_state == GPIO_PIN_RESET && key_up_last_state == GPIO_PIN_SET)
    {
        ...
        {
            last_tick_up = HAL_GetTick();

            // KEY_UP: 切换到故障检测模式
            fault_detection_flag = 1;
            basic_measurement_flag = 0; // 清零其他模式标志位
            sweep_freq_response_flag = 0;
            measure_r_out_flag = 0;
            current_system_state = FAULT_DETECTION_STATE;
        }
    }
    key_up_last_state = key_up_state;
}
