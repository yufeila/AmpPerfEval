...
void Basic_Measurement_Page_Update(void)
{
    ...
    LCD_Fill(38, 90, 230, 97, WHITE);  // 清除频率显示区域
    sprintf(dispBuff, "%.3f kΩ", R_in ); 
    LCD_ShowString(60, 90, 120, 12, 12, (uint8_t*)dispBuff);

}

static float CalculateRin(SignalAnalysisResult_t* res)
{
    float Rin = (V_s - res->adc_in_Result.amplitude/V_Rs_Gain)/(adc_in_Result.amplitude/V_Rs_Gain);
    return Rin;
}
