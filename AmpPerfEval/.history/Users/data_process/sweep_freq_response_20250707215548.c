...
{  
    ...
    if(++debug_counter % 10 == 0) {  // 每10次调用输出一次，避免刷屏
        ...
        // 通过UART输出调试信息（如果可用）或暂存到全局变量供Keil查看
        // printf("ADC: CH2[%lu,%lu,%lu] CH4[%lu,%lu,%lu] CH6[%lu,%lu,%lu]\n", 
        //        min_ch2, sum_ch2/100, max_ch2, min_ch4, sum_ch4/100, max_ch4, min_ch6, sum_ch6/100, max_ch6);
        
        // 暂存调试数据供Keil Watch窗口查看
        static volatile uint32_t debug_ch2_avg, debug_ch4_avg, debug_ch6_avg;
        static volatile uint32_t debug_ch2_range, debug_ch4_range, debug_ch6_range;
