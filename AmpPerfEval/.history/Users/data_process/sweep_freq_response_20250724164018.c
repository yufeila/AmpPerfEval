 ...
 * @retval 对应的频率值 (Hz)
 */
static float Calculate_Log_Frequency_Fault_Detection(uint8_t point_index)
{
    if (point_index >= FREQ_POINTS_FAULT_DETECTION) {
        return FREQ_STOP_FAULT_DETECTION;
    }
    
    // 对数分布：freq = FREQ_START * (FREQ_STOP/FREQ_START)^(index/(FREQ_POINTS-1))
    float log_ratio = (float)point_index / (float)(FREQ_POINTS_FAULT_DETECTION - 1);
    float frequency = FREQ_START_FAULT_DETECTION * powf(FREQ_STOP_FAULT_DETECTION / FREQ_START_FAULT_DETECTION, log_ratio);
...
void Fault_Detection(void)
{
    ...
    /* 调试信息 */
    printf("input_amp: %f, output_amp: %f, gain: %f, R_in: %f.\n", input_amp, output_amp, gain, R_in);
    if(fault_detected)
    {
    ...
    if(fault_detected)
    {
        // ---- 开始扫频 ----
        // 如果测量还未完成，继续扫频测量
        if (!measurement_complete && current_point < FREQ_POINTS_FAULT_DETECTION)
        {
            // 计算当前频率点
            float input_freq = Calculate_Log_Frequency_Fault_Detection(current_point);
        ...
        if (current_point >= FREQ_POINTS_FAULT_DETECTION)
        {
            measurement_complete = 1;
            float freq_3db = Find_3dB_Frequency(freq_response, FREQ_POINTS_FAULT_DETECTION);

            // 判断是电阻异常，还是电容异常
            float gain_variance = Calculate_Gain_Variance(freq_response, FREQ_POINTS_FAULT_DETECTION);
            ...
            if(fault_type == R_FAULT_TYPE)
            {
                ...
                if( fabs(data_at_1k.adc_dc_out_Result - 2.24f) < 0.1f && fabs(CalculateRin(&data_at_1k) - 14.68f) < 0.5f)
                {
                    LCD_ShowString(10, 60, 220, 16, 12, (uint8_t*)"R1 Open Circuit");
                }
                else if( fabs(data_at_1k.adc_dc_out_Result - 2.07f) < 0.1f)
                {
                    LCD_ShowString(10, 60, 220, 16, 12, (uint8_t*)"R1 Short Circuit");
                }
                else if( fabs(data_at_1k.adc_dc_out_Result - 1.12f) < 0.1f)
                {
                ...
                else if( fabs(data_at_1k.adc_dc_out_Result - 0.03f) < 0.01f)
                {
                ...
                {
                    LCD_ShowString(10, 60, 220, 16, 12, (uint8_t*)"Unknown Error");
                }
            }
            else if(fault_type == C_FAULT_TYPE)
            {
                if(fabs(freq_3db - 0.0f) < 100)
                {
                    LCD_ShowString(10, 60, 220, 16, 12, (uint8_t*)"C1 Open Circuit");
                }
                else if(fabs(freq_3db - 5000.0f) < 100)
                {
                    LCD_ShowString(10, 60, 220, 16, 12, (uint8_t*)"C1 Doubled");
                }
                else if(fabs(freq_3db - 138000.0f) < 100)
                {
                    LCD_ShowString(10, 60, 220, 16, 12, (uint8_t*)"C2 Doubled");  
                }
                else if(fabs(freq_3db - 192000.0f) < 100)
                {
                    LCD_ShowString(10, 60, 220, 16, 12, (uint8_t*)"C3 Open Circuit");
                }
                else if(fabs(freq_3db - 83000.0f) < 100)
                {
                    LCD_ShowString(10, 60, 220, 16, 12, (uint8_t*)"C3 Doubled");
                }
                else
                {
                    LCD_ShowString(10, 60, 220, 16, 12, (uint8_t*)"Unknown Error");
