...
{  
    ...
    if (current_system_state == SWEEP_FREQ_RESPONSE_STATE) {
        ...
        printf("Channel mapping: CH2(Vout)=%d, CH4(Vin)=%d, CH6(Vdc)=%d\r\n",
               adc_buffer[0], adc_buffer[1], adc_buffer[2]);
               
        printf("After Demux[0-2]: Vin=%.3f, Vout=%.3f, Vdc=%.3f\r\n",
               adc_in_buffer[0] + (adc_in_buffer[0] + adc_in_buffer[1] + adc_in_buffer[2])/3.0f, // 恢复DC后的Vin值
               adc_ac_out_buffer[0] + (adc_ac_out_buffer[0] + adc_ac_out_buffer[1] + adc_ac_out_buffer[2])/3.0f, // 恢复DC后的Vout值
               adc_dc_out_buffer[0]);
    }
                  ...
                  uint16_t len)
{
    ...
    {
        // 根据实际硬件连接修正通道映射：
        // src[3*i]   = ADC_CHANNEL_2 = 实际是Vout输出
        // src[3*i+1] = ADC_CHANNEL_4 = 实际是Vin输入  
        // src[3*i+2] = ADC_CHANNEL_6 = 实际是Vdc直流
        float volt_vout = (float)src[3*i] * ADC_LSB_VOLT;      // 实际是输出
        float volt_vin = (float)src[3*i+1] * ADC_LSB_VOLT;     // 实际是输入
        
        buf1[i] = volt_vin;   // buf1期望是输入，从src[3*i+1]取
        buf2[i] = volt_vout;  // buf2期望是输出，从src[3*i]取
        buf3[i] = (float)src[3*i+2] * ADC_LSB_VOLT;  // DC通道保持原样
        
        sum1 += volt_vin;   // 输入通道的DC分量
        sum2 += volt_vout;  // 输出通道的DC分量
    }
...
static void Measure_Single_Point(float frequency, FreqResponse_t* result, float fs)
{
    ...
    if (data_ready)
    {
        ...
        // *** 新增：扫频测量调试输出 ***
        printf("SWEEP[%02d]: Set=%.1fHz, Measured=%.1fHz/%.1fHz, Vin=%.3fV, Vout=%.3fV, Gain=%.2fdB, Fs=%.0fHz\r\n",
