...
{  
  ...
  static float adc_dc_out_buffer[FFT_SIZE];  // 通道3
    ...
    if (ADC_BufferReadyFlag == BUFFER_READY_FLAG_HALF)
    {
        ...
        // 分别处理3个子数组中的数据
        ProcessSampleData_F32(adc_in_buffer, adc_in_Result);
        ProcessSampleData_F32(adc_ac_out_buffer, adc_ac_out_Result);
        CalcArrayMean(adc_dc_out_buffer, adc_dc_out_Result);
