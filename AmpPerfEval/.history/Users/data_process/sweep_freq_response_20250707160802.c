...
static SpectrumResult_t v_open_drain_snapshot;
...
void Basic_Measurement(void)
{
    ...
    if(first_refresh)
    {
        ...
        // 启动ADC+DMA+TIM系统，配置到1000Hz对应的采样率，并获取实际采样率
    ...
    // 非阻塞式输出阻抗测量状态机
    Handle_Rout_Measurement_NonBlocking(current_fs);
}

/**
 * @brief 非阻塞式输出阻抗测量处理函数
 * @param fs 当前采样率
 * @retval None
 */
static void Handle_Rout_Measurement_NonBlocking(float fs)
{
    switch(rout_state)
    {
        case ROUT_IDLE:
        ...
        case ROUT_SNAPSHOT:
            // 获取开路电压快照，确保数据一致性
            v_open_drain_snapshot = data_at_1k.adc_ac_out_Result;
            
            // 验证快照数据有效性
            if(v_open_drain_snapshot.amplitude < 0.001f)
            {
                // 数据无效，显示错误并退出
                DisplayRout_Error("信号太弱");
        ...
        case ROUT_CALCULATING:
            {
                ...
                // 计算输出阻抗
                float R_out = CalculateRout(&v_out_avg, &v_open_drain_snapshot);
 ...
 * @retval None
 */
static void DisplayRout_Error(const char* error_msg)
{
    // 设置显示颜色
    POINT_COLOR = RED;
    BACK_COLOR = WHITE;
    ...
    // 显示错误消息
    LCD_ShowString(60, 190, 120, 12, 12, (uint8_t*)error_msg);
    
    // 可选：在下一行显示提示
    LCD_ShowString(60, 205, 120, 12, 12, (uint8_t*)"请重试");
}
 ...
 * @retval 实际采样频率
 */
static float Start_ADC_DMA_TIM_System(float frequency)
{
    // 1. 停止可能正在运行的ADC和定时器
