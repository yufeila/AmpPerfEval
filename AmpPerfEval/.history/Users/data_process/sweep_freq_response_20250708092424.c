...
{  
    ...
    // 【调试】检查数据分离后的结果
    if(debug_counter % 20 == 0) {  // 只在打印ADC统计时同时打印
        float sum_buf1 = 0.0f, sum_buf2 = 0.0f, sum_buf3 = 0.0f;
        for(int i = 0; i < 50; i++) {  // 检查前50个转换后的数据
            sum_buf1 += adc_in_buffer[i];
            sum_buf2 += adc_ac_out_buffer[i]; 
            sum_buf3 += adc_dc_out_buffer[i];
        }
        sprintf(debug_msg, "[Demux] Buf1: %.3fV, Buf2: %.3fV, Buf3: %.3fV\r\n", 
                sum_buf1/50.0f, sum_buf2/50.0f, sum_buf3/50.0f);
        HAL_UART_Transmit(&huart1, (uint8_t*)debug_msg, strlen(debug_msg), 100);
    }
