...
void Basic_Measurement(void)
{
    ...
    // 数据采集和处理
    Process_ADC_Data_F32(&data_at_1k.adc_in_Result, &data_at_1k.adc_ac_out_Result, &data_at_1k.adc_dc_out_Result, 1000.0f);
    ...
    if(measure_r_out_flag)
    {
        ...
        {
            Process_ADC_Data_F32(&v_out_with_load[i].adc_in_Result, &v_out_with_load[i].adc_ac_out_Result, &v_out_with_load[i].adc_dc_out_Result, 1000.0f);
        }
...
static void Measure_Single_Point(float frequency, FreqResponse_t* result)
{
    ...
    while (!data_ready && timeout < 1000)
    {
        data_ready = Process_ADC_Data_F32(&single_result.adc_in_Result, 
                                         ...
                                         &single_result.adc_dc_out_Result, frequency);
        if (!data_ready)
        {
            HAL_Delay(10);
            timeout += 10;
