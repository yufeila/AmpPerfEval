...
{
    return (float)(v_out_open_drain->adc_ac_out_Result.amplitude - v_out_with_load->adc_ac_out_Result.amplitude)/v_out_with_load->adc_ac_out_Result.amplitude * R_L;
}
 ...
 * @brief 更新基本测量页面显示
 */
void Basic_Measurement_Page_Update(void)  // 修正函数名
{
    // 设置颜色
    POINT_COLOR = DATA_COLOR;
    BACK_COLOR = WHITE;
...
static void DisplayRout(float Rout)
{
    LCD_Fill(60, 100, 230, 112, WHITE);
    sprintf(dispBuff, "%.3f kΩ", R_in); 
    LCD_ShowString(60, 160, 120, 12, 12, (uint8_t*)dispBuff);
}
