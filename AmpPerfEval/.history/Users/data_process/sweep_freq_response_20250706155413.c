...
// 全局变量声明
/* 基本参数显示 */
static SpectrumResult_t adc_in_Result;          // 全局存储最新的ADC_IN通道的FFT结果
static SpectrumResult_t adc_ac_out_Result;      // 全局存储最新的ADC_AC_OUT通道的FFT结果
static float adc_dc_out_Result;                 // 全局存储最新的ADC_DC_OUT通道的结果
...
void Display_Spectrum_Results(void)
{
    ...
    if (newDataProcessed || first_refresh) {
        ...
        ILI9341_Clear(0, LINE(4), LCD_X_LENGTH, 16);
        sprintf(dispBuff, "Vpp : %.3f V", Vpp);
        ILI9341_DispStringLine_EN(LINE(4), dispBuff);

        // Vout AC 显示
        ILI9341_DispStringLine_EN(LINE(6), "Vout AC Measuring Results");
        ...
        ILI9341_DispStringLine_EN(LINE(10), "Vout DC Measuring Results");
        ILI9341_Clear(0, LINE(11), LCD_X_LENGTH, 16);
...
{
  /* 根据需求调整 */
  const uint8_t process_interval = 3;   // 每隔 3 次执行一次 FFT
...
void Auto_Frequency_Response_Measurement(void)
{
    ...
    // 清除LCD显示测量状态
    ILI9341_Clear(0, 0, LCD_X_LENGTH, LCD_Y_LENGTH - 50);
...
static void Display_Frequency_Response_Curve(void)
{
    ...
    // 显示标题
    ILI9341_DispStringLine_EN(LINE(0), "Freq Response Measurement");
...
static void Draw_Coordinate_System(void)
{
    ...
    // X轴 (频率轴) - 在底部
    ILI9341_DrawLine(plot_x_start, plot_y_end, plot_x_end, plot_y_end);
    // X轴箭头 (右箭头)
    ILI9341_DrawLine(plot_x_end, plot_y_end, plot_x_end-ARROW_SIZE, plot_y_end-ARROW_SIZE/2);  // 上斜线
    ILI9341_DrawLine(plot_x_end, plot_y_end, plot_x_end-ARROW_SIZE, plot_y_end+ARROW_SIZE/2);  // 下斜线

    // Y轴 (增益轴) - 从底部到顶部
    ILI9341_DrawLine(plot_x_start, plot_y_start, plot_x_start, plot_y_end);
    // Y轴箭头 (上箭头)
    ILI9341_DrawLine(plot_x_start, plot_y_start, plot_x_start-ARROW_SIZE/2, plot_y_start+ARROW_SIZE);  // 左斜线
    ILI9341_DrawLine(plot_x_start, plot_y_start, plot_x_start+ARROW_SIZE/2, plot_y_start+ARROW_SIZE);  // 右斜线
    ...
    ILI9341_DispString_EN(plot_x_end-40, plot_y_end+25, "Freq(Hz)");  // X轴标签
    ILI9341_DispString_EN(5, plot_y_start-25, "Gain(dB)");            // Y轴标签
}
...
static void Display_Measurement_Info(void)
{
    ...
    ILI9341_DispStringLine_EN(LINE(14), info);
