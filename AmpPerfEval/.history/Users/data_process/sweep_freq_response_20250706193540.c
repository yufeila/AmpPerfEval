...
void Basic_Measurement_Page_Update(void)
{
    ...
    /* 输出直流部分 */
    LCD_Fill(38, 235, 230, 247, WHITE);  // 清除频率显示区域
    sprintf(dispBuff, "%.2f V", data_at_1k.adc_dc_out_Result.amplitude);
    LCD_ShowString(60, 235, 120, 12, 12, (uint8_t*)dispBuff);
}

static float CalculateRin(SignalAnalysisResult_t* res)
{
    // 错误：adc_in_Result 前面缺少 res->
