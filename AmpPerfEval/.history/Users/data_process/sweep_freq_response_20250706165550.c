...
void Basic_Measurement(void)
{
    ...
    if(measure_R_out)
    {
        SpectrumResult_t V_open_drain_out = data_at_1k;
        ...
        static SpectrumResult_t[PROCESS_ARRAY_SIZE_WITH_R_L];
        // 延时等待稳定
        HAL_Delay(1000);
 ...
 */
uint8_t Process_ADC_Data_F32(SpectrumResult_t* pRes1, SpectrumResult_t* pRes2, float* pRe3)
{  
  uint8_t new_data = 0;
  ...
  static float adc_dc_out_buffer[FFT_SIZE];  // 通道3
    ...
    if (ADC_BufferReadyFlag == BUFFER_READY_FLAG_HALF)
    {
        ...
        CalcArrayMean(adc_dc_out_buffer, pRe3);
