...
//外部变量声明
extern __IO  uint16_t adc_buffer[ADC_BUFFER_SIZE];
...
static uint8_t last_sweep_mode_state = 0;   // 记录上次扫频模式状态，用于检测模式切换：0=未激活，1=激活
...
void Basic_Measurement(void)
{
    ...
    last_sweep_mode_state = sweep_freq_response_flag;
...
void Auto_Frequency_Response_Measurement(void)
{
    ...
    if(sweep_freq_response_flag == 1 && last_sweep_mode_state == 0)
    {
        first_refresh = 1;  // 触发重新初始化
    }
    last_sweep_mode_state = sweep_freq_response_flag;  // 更新模式状态记录
