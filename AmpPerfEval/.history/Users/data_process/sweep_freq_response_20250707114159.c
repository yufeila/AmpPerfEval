 ...
 */
uint8_t Process_ADC_Data_F32(SpectrumResult_t* pRes1, SpectrumResult_t* pRes2, float* pRes3, uint16_t g_target_M)
{  
    //启动ADC和DMA
    //HAL_ADC_Start_DMA(&hadc1, adc_buf, FFT_SIZE*3);   // 2048点×3通道
  uint8_t new_data = 0;
  ...
  static float adc_dc_out_buffer[FFT_SIZE];  // 通道3
    ...
    if (ADC_BufferReadyFlag == BUFFER_READY_FLAG_HALF)
    {
        ...
        // 分别处理3个子数组中的数据
        ProcessSampleData_ByHint_F32(adc_in_buffer, pRes1, g_target_M);
        ProcessSampleData_ByHint_F32(adc_ac_out_buffer, pRes2, g_target_M);
...
void Auto_Frequency_Response_Measurement(void)
{
    if(first_fresh2)
    {
        ...
        static FreqResponse_t freq_response[FREQ_POINTS];
        volatile float g_target_freq   = 100.0f;  // 当前 sweep 点的名义频率 (Hz)
        volatile uint16_t g_target_M   = 1;        // 本点选中的抽取因子
...
{   
    ...
    g_target_freq = frequency;
    g_target_M    = choose_M(frequency);   // 计算抽取因子M
