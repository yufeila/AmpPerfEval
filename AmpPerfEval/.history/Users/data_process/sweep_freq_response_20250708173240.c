...
{  
    ...
    if (current_system_state == SWEEP_FREQ_RESPONSE_STATE) {
        ...
        // 显示分离后的纯AC信号和原始DC值
        printf("After Demux[0]: Vin_AC=%.3f, Vout_AC=%.3f, Vdc_raw=%.3f\r\n",
               adc_in_buffer[0],      // 已去除DC的Vin AC信号
               adc_ac_out_buffer[0],  // 已去除DC的Vout AC信号  
               adc_dc_out_buffer[0]); // 原始Vdc电压
    }
                  ...
                  uint16_t len)
{
    ...
    {
        // 修复通道映射：根据实际测试，前两个通道是反的
        float volt_ch1 = (float)src[3*i] * ADC_LSB_VOLT;      // 实际是Vout (ADC_CHANNEL_4)
        float volt_ch2 = (float)src[3*i+1] * ADC_LSB_VOLT;    // 实际是Vin (ADC_CHANNEL_2)
        
        buf1[i] = volt_ch2;  // adc_in_buffer = Vin (交换后)
        buf2[i] = volt_ch1;  // adc_ac_out_buffer = Vout (交换后)
        buf3[i] = (float)src[3*i+2] * ADC_LSB_VOLT;  // DC通道保持原样
        
        sum1 += volt_ch2;  // 累加Vin的值用于计算DC分量
        sum2 += volt_ch1;  // 累加Vout的值用于计算DC分量
    }
