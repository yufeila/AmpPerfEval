b#include "sweep_freq_response.h"
#include <stdio.h>
#include <string.h>
...
static void Draw_Coordinate_System(void)
{
    ...
    // 频率刻度 (对数刻度) - 在X轴上
    POINT_COLOR = WHITE;
    for(uint8_t i = 0; i <= 4; i++)  // 改为5个刻度点
    {
        uint16_t x = plot_x_start + i * (plot_x_end - plot_x_start) / 4;
        float freq = powf(10.0f, 2.0f + i * 0.75f);  // 100Hz, 316Hz, 1kHz, 3.16kHz, 10kHz, 31.6kHz, 100kHz
        ...
        if(freq < 1000)
            ...
            sprintf(label, "%.1fk", freq/1000.0f);
        else
            sprintf(label, "%.1fM", freq/1000000.0f);
        
        // 标签在X轴下方，使用12号字体
        POINT_COLOR = WHITE;
        BACK_COLOR = BLACK;
        LCD_ShowString(x-15, plot_y_end+8, 30, 12, 12, (uint8_t*)label);
    }
