 ...
 * @LastEditTime: 2025-07-08 15:37:28
...
**已完成修复**：
1. **频率有效性检查优化**：
   - 修改`ProcessSampleData_F32`函数，添加扫频模式检测逻辑
   - 扫频模式下大幅放宽频率变化检查限制（500Hz vs 原来的200Hz）
   - 扫频模式下禁用历史结果替换，保持FFT原始测量结果
   - 添加全局变量`g_sweep_running`用于标识扫频状态

2. **扫频状态管理**：
   - 在`Auto_Frequency_Response_Measurement`开始时设置`g_sweep_running = 1`
   - 在扫频完成或用户切换模式时设置`g_sweep_running = 0`
   - 在`sweep_freq_response.h`中声明外部变量

3. **参数范围进一步放宽**：
   - 幅度检查范围：0.001V ~ 10V（原来0.01V ~ 5V）
   - 频率检查范围：10Hz ~ 250kHz（原来50Hz ~ 220kHz）
   - 非扫频模式幅度变化容忍：10倍变化（原来5倍）
   - 非扫频模式频率变化容忍：500Hz（原来200Hz）

**预期效果**：
- 扫频时每个频点应该能正确显示实际测量的频率值
- 不再出现大部分频点被锁定在1000Hz的问题
- 保持非扫频模式下的异常值过滤功能

**下一步测试**：
需要编译并测试修复后的代码，验证扫频测量是否能正确输出每个频点的实际测量频率。
