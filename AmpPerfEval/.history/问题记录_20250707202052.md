 ...
 * @LastEditTime: 2025-07-07 20:19:12
...
#### 1. Error_Handler中断问题
**可能原因：**
- **ADC+DMA配置冲突**：扫频模式下频繁启停ADC可能导致DMA配置错误
- **定时器配置错误**：TIM2参数配置可能存在问题
- **内存越界**：DMA缓冲区或数组访问越界
- **HAL函数返回错误**：`HAL_ADC_Start_DMA`可能返回HAL_ERROR

#### 2. 信号测量显示0的问题
**分析：**
- **FFT未执行**：`spectrum_analysis`函数可能由于`PROCESS_INTERVAL`设置导致很少执行
- **ADC通道问题**：PA2, PA4的ADC配置可能有误
- **采样频率异常**：定时器配置导致采样频率过低或过高
- **数据处理逻辑**：`ProcessSampleData_F32`中的计数器逻辑可能有问题

#### 3. 定时器配置异常分析
```
状态1 (基本测量): PSC=0, ARR=0x833F (33599)
计算频率: 84MHz / (1 * 33600) ≈ 2.5kHz

状态2 (扫频测量): PSC=0x05 (5), ARR=0xDABF (56031) 
计算频率: 84MHz / (6 * 56032) ≈ 250Hz
```
**问题：** 扫频模式下采样频率过低，可能导致：
- ADC采样间隔过长
- DMA传输异常
...
2. **检查HAL函数返回值**
   ...
   HAL_StatusTypeDef status = HAL_ADC_Start_DMA(&hadc1, (uint32_t*)adc_buffer, BUF_SIZE);
   if (status != HAL_OK)
   {
       // 记录错误状态，不要调用Error_Handler()
       return 0;
   }
   ```

3. **临时禁用扫频模式测试**
   - 先注释掉扫频相关代码，确保基本测量模式稳定

4. **检查定时器配置函数**
   - 验证`Tim2_Config_AutoFs`函数的实现
   - 确保返回值合理（建议1kHz以上）

#### 深度调试方案：
1. **验证ADC配置**
   - 检查PA2, PA4, PA6的GPIO和ADC配置
   - 确认ADC通道映射正确

2. **检查FFT执行频率**
   - 调试`ProcessSampleData_F32`函数中的计数器
   - 确保FFT定期执行

3. **内存检查**
   - 验证`adc_buffer`, `freq_response`等数组边界
   - 检查栈溢出情况