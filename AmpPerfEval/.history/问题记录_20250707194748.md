 ...
 * @LastEditTime: 2025-07-07 19:47:48
...
1. ? **R_out测量结果保存问题**：现在切换模式后能正确显示之前的测量结果
2. ? **界面覆盖问题**：模式切换时完全清除上一模式的显示内容
3. ? **状态混乱问题**：分离了测量触发标志和显示状态
4. ? **代码逻辑混乱**：移除了冗余的状态追踪变量，简化了逻辑

### 待解决问题

1. ? **Vout(RL)实际值显示**：需要在测量时保存并显示真实的负载电压值
2. ? **频率测量和幅度测量不刷新**：需要进一步调试ADC采集问题
3. ? **扫频测量进度不均匀**：需要优化频率选择算法