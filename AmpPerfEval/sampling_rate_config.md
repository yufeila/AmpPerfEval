...
1. **采样率上限**: 实际测量上限可达259kHz，远高于之前估算的86kHz
2. **信号质量**: 高频时仍需考虑信号衰减和噪声
3. **采样率误差**: 定时器分频器限制可能导致实际采样率略有偏差
4. **FFT分析**: 使用实际采样率进行频谱分析，避免频率偏移
5. **扫频范围**: 理论上可以扫频到200kHz以上，但需要验证信号质量
...
```c
// 验证最大采样率
float max_fs = Tim2_Config_Channel_Fs(300000.0f);  // 尝试300kHz
printf("Max sampling rate: %.1f Hz\n", max_fs);  // 应该约为259kHz
```
