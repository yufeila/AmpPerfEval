...
void Basic_Measurement_Page_Update(void)  
{
    ...
    /* 开路增益显示 */
    LCD_Fill(100, 160, 230, 172, WHITE);
    //(V_s * 1e-3 * 0.5 - res->adc_in_Result.amplitude/V_Rs_Gain
    float gain = (data_at_1k.adc_ac_out_Result.amplitude * 11 * 47/247) / (V_s - data_at_1k.adc_in_Result.amplitude/V_Rs_Gain) ;
    sprintf(dispBuff, "%.3f V", gain, "(measured)");
    LCD_ShowString(100, 160, 120, 12, 12, (uint8_t*)dispBuff);
}
